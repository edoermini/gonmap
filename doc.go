...
For every scan it's necessary define a scan with scan := NewInitScan() and populate it with related functions or
directly with scan, err := NewScan([]string{"host1", "host2", ...}, []int{port1, port2, ...}, performance, vesionScan).
